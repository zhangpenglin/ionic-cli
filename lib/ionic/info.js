...
IonicTask.prototype.getIosDeployInfo = function() {
...
IonicTask.prototype.getIonicVersions = function(info) {
  ...
  var versionJsonPath = path.resolve('www/lib/ionic/version.json');
  var ionicBowerJsonPath = path.resolve('www/lib/ionic/bower.json');
  var versionFileExisted = true;

  try {
    ionicVersionFile = require(versionJsonPath);
  ...
  if (!versionFileExisted) {
    try {
      ionicVersionFile = require(ionicBowerJsonPath);
      ionicVersion = ionicVersionFile.version;
      info.ionic = ionicVersion;
    } catch (ex) { }
  }
...
IonicTask.prototype.getOsEnvironment = function(info) {
  ...
  switch(os.type()) {
    case 'Darwin':
      ...
      info.ios_deploy = self.getIosDeployInfo();
      break;
    case 'Windows_NT':
...
IonicTask.prototype.printInfo = function(info) {
  ...
  if(info.ios_deploy) {
    console.log('ios-deploy version:', info.ios_deploy);
  }
  
  console.log('\n');
}

IonicTask.prototype.checkRuntime = function checkRuntime() {
  var info = this.gatherInfo();
  var validRuntime = true;

  try {

    if(!semver.satisfies(info.node, requirements.node)) {
      var updateMessage = ['Please update your Node runtime to version >=', requirements.node].join(' ');
      console.log(updateMessage.red.bold);
      validRuntime = false;
    }

    if(!semver.satisfies(info.cordova, requirements.cordova)) {
      var updateMessage = ['Please update your Cordova CLI to version >=', requirements.cordova, '`npm install -g cordova`'].join(' ')
      console.log(updateMessage.red.bold);
      validRuntime = false;
    }
  } catch(ex) {
    console.log('There was an error checking your runtime environment: ', ex)
  }
